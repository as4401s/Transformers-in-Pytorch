...
class InputEmbeddings(nn.Module):
    """
    Converts input token IDs to dense vectors (embeddings).
    """
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # Create a vector for the positions (0, 1, 2, ..., seq_len-1)
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        
        # Denominator term for the sine and cosine functions
...
class LayerNormalization(nn.Module):
    """
    Normalizes the features for each token in the sequence.
    """
...
class FeedForwardBlock(nn.Module):
    """
    A simple two-layer feed-forward network applied to each token.
    """
...
class ResidualConnection(nn.Module):
    """
    Implements the residual connection (Add & Norm) step.
    """
...
class Encoder(nn.Module):
    """
    The Transformer Encoder, composed of a stack of EncoderBlocks.
    """
...
class Decoder(nn.Module):
    """
    The Transformer Decoder, composed of a stack of DecoderBlocks.
    """
    def __init__(self, features: int, layers: nn.ModuleList) -> None:
        super().__init__()
        self.layers = layers
...
class ProjectionLayer(nn.Module):
    """
    Projects the decoder output to the vocabulary size to get final probabilities.
    """
...
class Transformer(nn.Module):
    """
    The full Transformer model architecture.
    """
